...
void decodeMotorCmdBytes(const char cmd, MotorCommand &motor_left,
                         MotorCommand &motor_right) {
  const char non_scaled_intensity_right = (MOTOR_COMMAND_INTENSITY_MASK & cmd);
  motor_right.intensity =
      (255 * non_scaled_intensity_right) / MOTOR_COMMAND_INTENSITY_MASK;
  motor_right.rotate_forward = MOTOR_COMMAND_DIRECTION_MASK & cmd;

  const char cmd_shifted = cmd >> 4;
  const char non_scaled_intensity_left =
      (MOTOR_COMMAND_INTENSITY_MASK & cmd_shifted);
  motor_left.intensity =
      (255 * non_scaled_intensity_left) / MOTOR_COMMAND_INTENSITY_MASK;
...
char encodeMotorCmdToByte(const MotorCommand &cmd) {
  ...
  const char direction_bit =
      (cmd.rotate_forward) ? MOTOR_COMMAND_DIRECTION_MASK : 0b00000000;
  const char intensity_byte =
      MOTOR_COMMAND_INTENSITY_MASK &
      ((MOTOR_COMMAND_INTENSITY_MASK * cmd.intensity) / 255);
  return direction_bit | intensity_byte;
}

char encodeMotorCmdToByte(const MotorCommand &cmd_left,
                          const MotorCommand &cmd_right) {
  const char cmd_byte_left =
      (0b00001111 & robin_firmware::encodeMotorCmdToByte(cmd_left)) << 4;
  const char cmd_byte_right =
      (0b00001111 & robin_firmware::encodeMotorCmdToByte(cmd_right));
